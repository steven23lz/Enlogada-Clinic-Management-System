/**
 * Additive migration [1.22.0] — evidence for an HMO claim.
 *
...
async function main() {
  ...
  logger.info(rollback
    ? '[1.22.0] ROLLBACK — removing HMO card evidence columns…'
    : '[1.22.0] Adding HMO card evidence to hmo_requests…');
  ...
  if (rollback) {
    logger.info('[1.22.0] Rolled back. Card images remain on disk under backend/uploads/hmo-cards/.');
    process.exit(0);
  }
  ...
  logger.info(`[1.22.0] Done. ${total} request(s): ${withImage} with an image, ${withVerifier} staff-verified.`);
