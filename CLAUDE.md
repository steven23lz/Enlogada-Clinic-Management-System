...
cd frontend && npm test        # 420 Playwright E2E, ~11m, needs both dev servers
```
...
There **is** an automated end-to-end suite: `frontend/tests/e2e/` holds 63 Playwright specs (420 tests, ~11m) run with `npm test` (or `npm run test:ui`) from `frontend/`. It assumes **both dev servers are already running** and hits the real database — see `frontend/tests/e2e/README.md`. See the tiers above.
