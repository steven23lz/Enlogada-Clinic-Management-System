...
15. **New chart-only color: `#2563eb` (2026-08-12, UI/UX Modernization Phase 5).** Installed `recharts` and replaced the hand-rolled `div`-bar "charts" (duplicated across `admin/ReportsOverview.jsx` and `AdminDashboard.jsx`) with real components: `frontend/src/components/charts/RevenueTrendChart.jsx` (single-series area/line, brand green `#769046` — no new color needed, one series carries no categorical-distinction burden) and `CategoryVolumeChart.jsx` (categorical horizontal bar, one color per service category). For the categorical chart, navy (`#34466b`) was tried first as the second color and rejected — at this app's UI scale navy reads as ink/dark-surface, not as a data-series fill, so it muddies against the white card background rather than reading as a distinct category. `#2563eb` (a blue with adequate lightness/chroma to read as a chart mark, and enough separation from the brand green to stay distinguishable under common color-vision deficiencies) was added as a **chart-only** accent — not a general-purpose UI color, not to be used on buttons/badges/text. Fixed categorical order, never reassigned when the category list changes: `#769046` (green) → `#2563eb` (blue) → `#d97706` (amber, already used as `MetricCard tone="amber"`) → `#059669` (emerald, already used as `tone="emerald"`). Indigo (`#4f46e5`/`indigo-500`, the color the old hand-rolled Service Volume bar used) is deliberately excluded from this palette — `status-badge.jsx` already reserves indigo as the semantic "Processing" status color (#10), and reusing it as a generic 4th category color would blur that meaning the first time a chart and a status badge appear on the same screen.
