...
cd frontend && npx playwright test                  # expect: 420 pass, 0 skipped
```
...
| Frontend unit | **90 passed** |
| Playwright E2E | **420 passed**, **0 skipped** (run with `--timeout=90000`, see §0) |
...
| Question | Where |
|---|---|
| *Which requirement does this implement? Was it tested?* | `.agents/TRACEABILITY.md` — 66 feature rows, MODULE → … → TEST → APPROVAL |
...
*Baseline recorded 2026-09-14, at the `[1.79.0]` commit. Re-measure and update the numbers in §1 whenever the suite
